...
if (isset($_GET["trip-start"]) && isset($_GET["trip-end"])) {
    ...
    $_SESSION['end'] = $end;
    $sql = "SELECT r.*, COALESCE(p.URL, 'https://picsum.photos/seed/WsPyhTrC/640/480') AS URL FROM room r INNER JOIN photo p ON r.id = p.room_id WHERE r.status = true AND r.discount = 0 AND NOT EXISTS (	SELECT 1 FROM booking b WHERE r.id = b.room_id AND (('$start' BETWEEN b.check_in AND b.check_out) OR ('$end' BETWEEN b.check_in AND b.check_out) OR (b.check_in BETWEEN '$start' AND '$end') OR (b.check_out BETWEEN '$start' AND '$end')));";
} else {
    $sql = "SELECT r.*, p.URL FROM room r LEFT JOIN photo p ON r.id = p.room_id WHERE r.status = true AND r.discount = 0;";
}
